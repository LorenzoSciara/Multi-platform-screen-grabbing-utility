...
pub fn modify(screen_result: Option<RgbaImage>, draw: Draw, draw_text: String, screen_result_backup: Option<RgbaImage>, color_slider_value: u8) -> Element<'static, Message> {
    ...
    if draw == Draw::Text && screen_result == screen_result_backup{
        controlModify = row![free_draw_button, circle_draw_button, arrow_draw_button, text_draw_button, text_draw_text].spacing(20);
    }
    else if draw == Draw::Text && screen_result != screen_result_backup{
        controlModify = row![free_draw_button, circle_draw_button, arrow_draw_button, text_draw_button, text_draw_text, clear_button].spacing(20);
    }
    else if draw != Draw::Text && screen_result != screen_result_backup{
        controlModify = row![free_draw_button, circle_draw_button, arrow_draw_button, text_draw_button, clear_button].spacing(20);
    }
    else{
        controlModify = row![free_draw_button, circle_draw_button, arrow_draw_button, text_draw_button].spacing(20);
    }
    match screen_result {
        Some(screen) => {
